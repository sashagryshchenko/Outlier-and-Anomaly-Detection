...
We see that by only considering the data where the delays are over 15 minutes, the quantiles have shifted, potentially helping us identify the "true" outliers. This may be the case becasue the negative values skew the data, but the negative values are simply early arrivals, so they can be dealt with separately. We can begin by using the HDBSCAN method, which is a hierarchical clustering algorithm meant to group the data and in that way, find outliers (those valeus that do not belong to a cluster). We will first try to find outliers for the delay variables for the entire dataset and then only consider values greater or equal to than 15.
...
indz <- as.numeric(rownames(z[which(hdbscanResult$cluster==0),]))
hdbscanResult
hullplot(z, hdbscanResult, xlab="Departure Delays", ylab="Arrival Delays",pch=20)
```

Above is the plot for the results of HDBSCAN for the entire dataset. The black points are the outliers (as identified by the method).

```{r}
numvars15 <- flights[which(flights$ARR_DEL15==1),c(6,7)]
z15 <- numvars15[sample(nrow(numvars15),30000),]
hdbscanResult15 <- hdbscan(z15, minPts=10)
indz15 <- as.numeric(rownames(z15[which(hdbscanResult15$cluster==0),]))
hdbscanResult15
hullplot(z15, hdbscanResult15, xlab="Departure Delays", ylab="Arrival Delays",pch=20)
```
...
ind <- unlist(indz)
# flights[unique(ind[duplicated(ind)]),]

colors <- c("Inliers" = "blue", "Outliers" = "red")
ggplot() +
  labs(x = "Day of Week", y = "Delay in Departure") +
  theme_classic() +
  geom_point(data=flights[unique(ind),],
             aes(x=DEP_DELAY,y=ARR_DELAY,color="Inliers"),
             alpha=0.2,shape=20) +
  geom_point(data=flights[-unique(ind),],
             aes(x=DEP_DELAY,y=ARR_DELAY,color="Outliers"),
             alpha=0.2,shape=20) +
  labs(colour="Legend")
...
ggplot() +
  labs(x = "Delay in Departure", y = "Delay in Arrival",color = "Legend") +
  theme_classic() +
  geom_point(data = flights[-ind,], aes(x = DEP_DELAY, y = ARR_DELAY,color="Inliers"),alpha=0.1) +
  geom_point(data = flights[ind,], aes(x = DEP_DELAY, y = ARR_DELAY,color="Outliers"),alpha=0.2) +
